    ...
    widgets = [
      ...
      },
      {
        ...
        properties = {
          ...
          metrics = [
            ["AWS/Lambda", "ConcurrentExecutions", "FunctionName", local.iot_dashboard.lambda_function, { stat = "Maximum", label = "Concurrent Execs" }],
            [".", "Throttles", ".", ".", { stat = "Sum", label = "Throttles" }]
          ]
          view    = "timeSeries"
          stacked = false
      ...
      },
      {
        ...
        height = 6
        properties = {
          title = "Lambda - Concurrency (Main & DLQ)"
          metrics = [
            ["AWS/Lambda", "ConcurrentExecutions", "FunctionName", local.iot_dashboard.lambda_function, { stat = "Maximum", label = "Main Concurrent" }],
            [".", "ConcurrentExecutions", "FunctionName", local.iot_dashboard.dlq_lambda, { stat = "Maximum", label = "DLQ Concurrent" }]
          ]
          view    = "timeSeries"
          stacked = false
          region  = var.region
